...
5. **Lack of Automated Testing and Dynamic Seeding Verification:**
   - *Challenge:* Relying solely on manual cURL endpoints is highly error-prone and does not programmatically verify that complex dynamic prioritization rules (such as category percentage discount vs product-level fixed discount overlaps) resolve mathematically correctly. It also fails to automatically confirm Redis Cache-Aside lookup and database connection pool health.
   - *Resolution:* Engineered an automated database seeding engine (`src/seed.ts`) and a comprehensive integration test suite (`src/test.ts`). Executing `npm run test` programmatically boots the Redis client, pulls database rows, resolves effective dynamic prices for distinct overlapping coupon conflicts, and asserts Redis caching integrity, establishing a bulletproof 4/4 verification gate.
